...
path <- c("/Volumes/rstor-henao_lab/Pablo/CFU_Pipeline/SVX_Boost2X_Spleen_Day90_cfu_numbers.xlsx")
...
spleenD90tidy_cfu_meta_joined  <- inner_join(meta, tidy_cfu_data) %>%
  group_by(group) %>% 
...
spleenD90tidy_cfu_meta_joined 

write.csv(spleenD90tidy_cfu_meta_joined, "spleenD90tidy_cfu_meta_joined.csv", row.names = FALSE)
...
labs (y="Log10 CFU", fill = "Vaccine") +
  ggtitle("SolaVAX-TB 2x Boost Day 90 Spleen CFU       Df:5") +
  ylim(2, 6)
```
