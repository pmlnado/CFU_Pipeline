...
tidy_cfu_data <- all_data %>%
  mutate(dilution = str_extract(dilution, "[0-9]+"),
         dilution = as.numeric(dilution)) %>%
  mutate(CFUs = as.numeric(CFUs)) %>%
  filter(CFUs >= 1, CFUs <= 150) %>%
  group_by(group, mouse, organ, sex, day) %>%
  slice(which.min(ifelse(CFUs >= 1 & CFUs <= 150, dilution, Inf))) %>%
  ungroup()
...
lungoption1tidy_cfu_meta_joined  <- inner_join(meta, tidy_cfu_data) %>%
  group_by(group) %>% 
  mutate(CFUs_whole = ((CFUs * dilution_factor^dilution/volume_plated_ml) * total_resuspension_mL)/(0.50)) %>%
...
write.csv(lungoption1tidy_cfu_meta_joined, "lungoption1tidy50_cfu_meta_joined.csv", row.names = FALSE)
...
anova_result <- aov(log10(CFUs_whole) ~ group, data = tidy_cfu_meta_joined %>% filter(day == "14"))
...
female_df <- tidy_cfu_meta_joined %>%
  filter(sex == "female" & day == "14")

male_df <- tidy_cfu_meta_joined %>%
  filter(sex == "male" & day == "14")
...
#USE FOR INDIVIDUAL DAYS
female_df <- tidy_cfu_meta_joined %>%
  filter(sex == "female" & day == "90")

male_df <- tidy_cfu_meta_joined %>%
  filter(sex == "male" & day == "90")
