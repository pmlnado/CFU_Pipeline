...
path <- c("/Volumes/rstor-henao_lab/Pablo/CFU_Pipeline/CFU_Test_Data.xlsx")
...
ggplot(tidy_cfu_meta_joined, aes(x = Vaccine, y = log10(CFUs_whole), fill = Vaccine, color = Vaccine)) +
geom_point(size = 5, shape = 21, alpha = 0.8, na.rm = T, colour = "black", 
           cex = 1.5, position = position_dodge(width = .75)) + 
  geom_boxplot(aes(fill = Vaccine), alpha = 0.3) + facet_wrap(~ sex) + 
