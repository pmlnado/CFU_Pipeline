...
path <- c("/Volumes/rstor-henao_lab/Pablo/CFU_Pipeline/IMPAcTB_Spleen_Option1/IMPACTB_CFU_Op1_AllDays_Spleen.xlsx")
...
spleenoption1tidy_cfu_meta_joined  <- inner_join(meta, tidy_cfu_data) %>%
  group_by(group) %>% 
  mutate(CFUs_whole = ((CFUs * dilution_factor^dilution/volume_plated_ml) * total_resuspension_mL)/.50) %>%
  dplyr::select(organ, count_date, who_plated, who_counted, group, sex, day, mouse, dilution, 
         CFUs, CFUs_whole) %>% 
  ungroup()
...
spleenoption1tidy_cfu_meta_joined

write.csv(spleenoption1tidy_cfu_meta_joined, "spleenoption1tidy_cfu_meta_joined.csv", row.names = FALSE)
...
#USE FOR INDIVIDUAL DAYS
female_df <- tidy_cfu_meta_joined %>%
  filter(sex == "female" & day == "56")

male_df <- tidy_cfu_meta_joined %>%
  filter(sex == "male" & day == "56")
